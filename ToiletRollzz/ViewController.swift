...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let anchor = AnchorEntity(plane: .horizontal, minimumBounds:
            [0.2, 0.2])
        ...
        var cards: [Entity] = []
        for _ in 1...9{
            let box = MeshResource.generateBox(width: 0.05, height: 0.04, depth: 0.02 )
        ...
        for (index, card) in cards.enumerated() {
            let x = Float(index % 3) - 1.5
            let z = Float(index / 3) - 1.5
        ...
        let occlusionBox = ModelEntity(mesh: occlusionBoxMesh, materials:
            [ OcclusionMaterial()])
        ...
        .collect()
            .sink(receiveCompletion:{error in
                ...
                for entity in entities{
                    entity.setScale(SIMD3<Float>(0.004, 0.004, 0.004),
                                    relativeTo:anchor)
                ...
                for (index, object) in objects.enumerated(){
                    ...
                    cards[index].transform.rotation = simd_quatf(angle:
                        .pi, axis: [1, 0, 0])
                }
